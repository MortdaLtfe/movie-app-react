...
const WatchList = () => {
  ...
  const handelAddToWatched = m => {
      var newArray = watchList.filter(p => p.imdbID !== m.imdbID);
    if (watched.find(o => (o.imdbID = m.imdbID))) {
      return setWatchList(newArray);
    }
    
    setWatchList(newArray);
    setWatched([...watched, m]);
  };
  const handelRemove = m => {
    const newArray = watchList.filter(o => o.imdbID !== m.imdbID);
    setWatchList(newArray);
  };
  return (
